...
int main (int argc, char *argv[] ){
    ...
    fprintf(gp, "set output 'square_expand.gif'\n");
    ...
    for(i=0; i<NEURON_LEARN; i++){
        ...
        fprintf(gp, "set title 't = %d/%d sigma =%lf alpha = %lf'\n",i*SUB_NEURON_LEARN, NEURON_LEARN*SUB_NEURON_LEARN,SIGMA,ALPHA);
    ...
    for(i=0; i<NEURON_X; i++){
        for(j=0; j<NEURON_Y; j++){
            for(k=0; k<IN_SIGNAL; k++){
                m[i][j][k] = (((float )rand()/RAND_MAX*2.0 )-1.0 )*0.001;
            }
